...
export default function MultipleChoice({ exercise, onAnswer, showHint, attempt, setAttempt }) {
  ...
  const feedback = useMemo(() => {
    if (!submitted) return "";
    return isCorrect ? "Correct." : `Incorrect. Correct answer: ${correctAnswer}`;
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-slate-500">Choose the best answer</p>
      ...
      <div className="space-y-3">
        {options.map((option, index) => {
          ...
          const showFeedback = submitted && isSelected && optionFeedback[index];
              ...
              <div>
                ...
                {showFeedback ? (
                  <p className="mt-1 text-xs text-slate-500">
                    {renderInlineMarkdown(optionFeedback[index])}
                  </p>
                ) : null}
              </div>
            </button>
