...
export default function FillBlank({ exercise, onAnswer, showHint, attempt, setAttempt }) {
  ...
  const question = exercise?.question ?? "Fill in: ___";
  let parts = [question, ""];
  const firstPlaceholder = question.match(/\{\d+\}/);
  if (firstPlaceholder) {
    const before = question.slice(0, firstPlaceholder.index);
    const afterRaw = question.slice(
      firstPlaceholder.index + firstPlaceholder[0].length
    );
    const after = afterRaw.replace(/\{\d+\}/g, "");
    parts = [before, after];
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-slate-500">Fill in the blank</p>
      ...
      <div className="text-left">
        <div className="flex flex-wrap items-center gap-2 text-slate-800 text-base sm:text-xl font-normal leading-relaxed">
          <span className="min-w-0">{renderInlineMarkdown(parts[0])}</span>
          ...
          <input
            ...
            className={[
              "border rounded-xl px-3 py-2 text-center font-semibold",
              "min-w-[8ch] sm:min-w-[10ch] md:min-w-[12ch] max-w-full",
              "w-auto",
            ...
            style={{ width: `${Math.max(9, Math.min(18, (answer ?? "").length + 2))}ch` }}
            placeholder="..."
            disabled={submitted}
            onKeyDown={(e) => {
          ...
          <span className="min-w-0">{renderInlineMarkdown(parts[1])}</span>
        </div>
      </div>
